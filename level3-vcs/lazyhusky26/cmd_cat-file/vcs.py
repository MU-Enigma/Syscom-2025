...
def cmd_cat_file(sha1):
    ...
    try:
        ...
        print(f"Error decompressing object {sha1}: {e}", file=sys.stderr)
        sys.exit(1)
    ...
    if len(content) != size:
        print(f"Warning: size mismatch in object {sha1} (expected {size}, got {len(content)})", file=sys.stderr)
